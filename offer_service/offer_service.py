...
def process():
    ...
    if customer.get("LETTER_TYPE", "").lower() != "offer":
        return "This service only handles offer letters", 400

    try:
        # Read template
        with open("templates/offer_template.txt", "r", encoding="utf-8") as file:
            template = file.read()
        ...
        template = template.replace("{{OFFER_TYPE}}", str(customer.get("OFFER_TYPE", "")))
        template = template.replace("{{CREDIT_LIMIT}}", str(customer.get("CREDIT_LIMIT", "")))
        ...
        filename = f"offer_{first_name}_{last_name}.txt"
        ...
        customer["service_processed"] = "offer_service"
        collection.insert_one(customer)

        print(f"Offer letter created: {output_path}")
        return f"Offer letter created: {output_path}", 200

    except FileNotFoundError:
        return "Offer template file not found", 500
